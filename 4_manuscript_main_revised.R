...
ggsave(last_plot(), file = file.path(dir_report, "fig4_intrinsic effects.png"),
       width =  19, height = 6.3,
       units = "cm",
       dpi = 1000)

## figure 5 - growth temporal trend (year + cohort random effect) ----
...
# year
m3_year <- lmer(log.increment ~ 1 + log.age + log.aac + method2 +
                  (1 | fishid) + (1 | pop.year), data = data, REML = T)

pred_year <- ranef(m3_year)$`pop.year` 
pred_year_se <- as.data.frame(arm::se.ranef(m3_year)$`pop.year`)
...
# cohort
m3_cohort <- lmer(log.increment ~ 1 + log.age + log.aac + method2 +
                  (1 | fishid) + (1 | pop.cohort), data = data, REML = T)

pred_cohort <- ranef(m3_cohort)$`pop.cohort` 
pred_cohort_se <- as.data.frame(arm::se.ranef(m3_cohort)$`pop.cohort`)

pred_cohort <- pred_cohort %>%
  mutate(pop = sub(":.*", "", rownames(pred_cohort)),
         cohort = as.numeric(sub(".*:", "", rownames(pred_cohort))),
         intercept = `(Intercept)`,
         intercept_se = pred_cohort_se$`(Intercept)`) %>%
  left_join(df_pop)

#### plot
ggplot() + 
  geom_line(data = pred_year, 
            aes(x = year, y = intercept)) +
  geom_ribbon(data = pred_year,
              aes(x = year, 
                  ymin = intercept - intercept_se*1.96,
                  ymax = intercept + intercept_se*1.96),
              alpha = 0.5) +
...
ggplot() + 
  geom_point(data = pred_cohort, 
            aes(x = cohort, y = intercept),
            size = rel(0.8)) +
  geom_linerange(data = pred_cohort,
              aes(x = cohort, 
                  ymin = intercept - intercept_se*1.96,
                  ymax = intercept + intercept_se*1.96),
              alpha = 0.5) +
...
ggsave(last_plot(), file = file.path(dir_report, "fig5_growth temporal trend.pdf"),
       ...
       units = "cm") 
ggsave(last_plot(), file = file.path(dir_report, "fig5_growth temporal trend.png"),
       width =  19, height = 12.6,
       units = "cm",
       dpi = 1000)

## figure 5 SI - growth temporal trend (year random effect) ----
# year random intercept + log.age slope (1 + log.age | pop.year) 
...
for (i in 1:length(list_model)) {
  ...
  pred_temp <- as.data.frame(Effect(c("c.temp_between"), 
                                    model, 
                                    xlevels = list(c.temp_between = seq(-1, 1, 0.1)))) %>%
    mutate(source = temp_source)
...
ggsave(last_plot(), file = file.path(dir_report, "fig6_temp effects.pdf"),
       ...
       width = 9, height = 7,
       units = "cm")
ggsave(last_plot(), file = file.path(dir_report, "fig6_temp effects.png"),
       width = 9, height = 7, #9,26
       units = "cm",
       dpi = 1000)
...
list_model <- list("oras5" = m5_nu_oras5)
...
ggsave(last_plot(), file = file.path(dir_report, "fig7_extrinsic effects.pdf"),
       ...
       width =  9, height = 17,
       units = "cm")
ggsave(last_plot(), file = file.path(dir_report, "fig7_extrinsic effects.png"),
       width =  9, height = 16,
       units = "cm",
       dpi = 1000)
...
for (i in 1:length(list_model)) {
  ...
  pred_temp <- as.data.frame(Effect(c("log.age", "c.temp_within", "f"), 
                               model, 
                               xlevels = list(log.age = unique(data$log.age),
                                              c.temp_within = seq(-1, 1, 0.1),
                                              f = df_f$f))) %>%
...
ggsave(last_plot(), file = file.path(dir_report, "fig8_temp vs env.pdf"),
       ...
       width = 19, height = 19,
       units = "cm")
ggsave(last_plot(), file = file.path(dir_report, "fig8_temp vs env.png"),
       width = 19, height = 19,
       units = "cm",
       dpi = 1000)
...
ggsave(last_plot(), file = file.path(dir_report, "fig9_variance ratio.pdf"),
       ...
       width = 9, height = 6.3,
       units = "cm")
ggsave(last_plot(), file = file.path(dir_report, "fig9_variance ratio.png"),
       width = 9, height = 6.3,
       units = "cm",
       dpi = 1000)
...
## table 3 - intrinsic, extrinsic, extrinsic extended model (without nutrient) ----
...
# summary table
tab_model(m3, m4_oras5, m5_oras5, 
          show.se = TRUE, 
          show.ci = NULL,
          #show.ci = 0.95, 
          show.p = TRUE,
          ...
          dv.labels = c("Intrinsic model", 
                        "Population-level extrinsic model", 
                        "Individual-level extrinsic model"),
          order.terms = c(1,2,3,4,5,8,6,7,9,12,11,10),
          ...
          digits.rsq = 2,
          CSS = css_list,
          file = file.path(dir_report, "table3_model_summary.html")
)
